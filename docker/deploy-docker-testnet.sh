#!/bin/bash
echo "SETTING UP TESTNET CONFIGURATION FOR ION"
...
DATA_VOL=$bitcoinDataDirectory DB_VOL=$coreDataDirectory IPFS_VOL=$ipfsDataDirectory docker-compose -f docker-compose.testnet.yml up --no-start

#start IPFS and Mongo. This gives IPFS time to start finding peers
docker start mongo
docker start ipfs 
#start bitcoin test, this can take 1 hours to complete, the script will wait for it to finish
#start ion-bitcoin, this can take 1 hour
...
echo "
{
  ...
  \"bitcoinPeerUri\": \"http://bitcoin-core-testnet:18332\",
  \"bitcoinRpcUsername\": \"admin\",
  \"bitcoinRpcPassword\": \"$password\",
  \"bitcoinWalletOrImportString\": \"cMpfxWq8J9psM6e12ZYFe287YdWAfwGo198UbGHy9tVmaupJZnzK\",
...
}" > ../json/testnet-bitcoin-docker-config.json

echo "Starting up bitcoin-node service"

# start the docker bitcoin node
docker start bitcoin-core-testnet
sleep 3
echo -ne "Bitcoin-core-testnet started, please wait for this to complete a sync before proceeding
Run \"docker logs -f bitcoin-core-testnet\" to tail the current logs from the node in another session to track progress
...
Please be patient. It takes a minute before the syncing starts and after that it can take up to 1 hour to download the entire database.
\n\n"
...
docker start ion-bitcoin-testnet
...
docker start ion-core-testnet

echo -ne "Congratulations. Your ION node is running connected to bitcoin testnet. Happy resolving"
read
